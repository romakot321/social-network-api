...
async def creator_details_page(request: Request, account_uow: AccountUoWDepend, task_adapter: ReportTaskAdapterDepend, task_uow: TaskUoWDepend, account_id: UUID, dto: CreatorVideosListParamsDTO = Depends()):
    ...
    report = await MakeReportUseCase(task_adapter).execute(account_id, ReportMakeDTO(from_datetime=from_datetime, to_datetime=datetime.datetime.now() - datetime.timedelta(days=from_datetime.weekday())))
    return templates.TemplateResponse("creator.html", {"request": request, "account": account, "videos": videos, "report": report})
